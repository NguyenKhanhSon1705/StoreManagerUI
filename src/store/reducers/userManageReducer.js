...
const userManage = (state = initState , action) =>{
    switch(action.type){
        ...
        case actionTypes.GET_USER_OF_TREE_BY_ID:{
            return {
                ...state,
                listUser: mergeTreeData(state.listUser ,action.payload.parentId,action.payload.data )}  //add selected user to state
        }
        ...
        case actionTypes.LOOK_USER: {
          return {
             ...state,
              listUser: removeNodeById(state.listUser, action.payload.data)  // remove user from tree
          }
        }
        default:
            return state
